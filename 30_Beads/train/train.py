...
def train(model, train_loader, val_loader, optimizer, config):
    ...
    for epoch in range(config.n_epochs):
        model.train()
        total_loss = total_recon = total_kl = total_bond_loss = total_angle_loss = 0
        ...
        for (x_batch,) in train_loader:
            ...
            loss, recon_loss, kl_div, bond_loss, angle_loss = vae_loss(x_batch, x_hat, mu, logvar, P=30, num_atoms=3)
        ...
        with torch.no_grad():
            for (x_batch,) in val_loader:
                x_batch = x_batch.to(config.device)
                x_hat, mu, logvar = model(x_batch)
                loss, _, _, _, _ = vae_loss(x_batch, x_hat, mu, logvar, P=30, num_atoms=3)
                val_loss += loss.item()
