...
np.savetxt("latent_mu_30Beads_new.csv", mu, delimiter=",")
...
np.savetxt("generated_norm_30Beads_new.csv", x_gen_norm, delimiter=",")
np.savetxt("generated_denorm_30Beads_new.csv", x_gen_denorm, delimiter=",")
...
def width_table(dist_original, dist_recon, dist_generated, ddof=1):
    ...
    print("\n=== Distribution widths ===\n")
    print(
        f"{'Quantity':10s} | {'Set':15s} | "
        f"{'Variance':>12s} | {'Std':>10s} | {'IQR':>10s}"
    )
    print("-" * 70)

    for i, quantity in enumerate(names):
        for label, dist in sets:
            x = np.asarray(dist[i])

            variance = np.var(x, ddof=ddof)
            standard_deviation = np.std(x, ddof=ddof)

            q25, q75 = np.percentile(x, [25, 75])
            iqr = q75 - q25

            print(
                f"{quantity:10s} | {label:15s} | "
                f"{variance:12.5f} | "
                f"{standard_deviation:10.5f} | "
                f"{iqr:10.5f}"
            )
