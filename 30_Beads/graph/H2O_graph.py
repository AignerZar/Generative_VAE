...
def build_node_features(batch_size, P, num_atoms, device):
    ...
    atom_types = []
    for b in range(P):
        atom_types.extend([0, 1, 1]) #0=O, 1= H # hier sollte es eigentlich sein ([1,0,1]) -> instead of [0, 1, 1]
    ...
    # now we differ if we only have one bead -> do not need a feature so 0 should be enough
    if P > 1:
